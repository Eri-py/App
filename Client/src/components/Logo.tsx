...
export function LogoWithName({ size, align }: LogoWithNameProps) {
  const logoWithName = (iconSize: string, textSize: number) => {
    return (
      <Stack direction="row" alignSelf={align ?? "unset"} gap={0.5}>
        <Logo width={iconSize} />
        <Typography color="primary" fontSize={textSize} fontWeight={700}>
          App
        </Typography>
      </Stack>
    );
